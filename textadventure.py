    ...
    while True:
        ...
        if position==32:
            ...
            if lighter==False:
                ...
                position=choose(3)
        if position==19:
            print("You are in Mr. Straut's room. While you liked him well enough, his room seems so vulnerable to a little mindless mayhem.")          #Sorry Mr. Straut
            things(19)
            print("1. Leave room\n2. Item")
            position=choose(2)
        if position==24:
...
def option (instruct):                  #the biggest function in my code. It takes the room that you are in and the option passed to it, then tells you what happens. There isn't a great way that I can think of to condense this significantly, so here it is. It's a lot of semi-boring code.
    ...
    if position==31:
        ...
        if instruct==5:
            if kippcad==False:
                ...
                print("You already took that. You hooligan.")           #wayward souls reference. 8th grade was a good year.
        return "retry"
    if position==23:
        global physbook
        if instruct==1:
    ...
    if position==101:
        if instruct==1:
            ...
            return 27
        if instruct==2:
            print("You look at the most recent changes. It looks like a 'DArtis' has been changing his grades. Shameful.")           #hi, it's me, the easter egg
        if instruct==3:
            return item()
        if instruct==4:
            if lockpick==False:
...
def bag():      #uses an item, then removes it from your temporary inventory. The temporary inventory is one created at the start of the fight that contains all useable items in your bag.
    ...
    if ituse=="Mrs. Gerstein's Searcher of Seeking":
        ...
        textbox(f"{shortname} located a weak point!")
